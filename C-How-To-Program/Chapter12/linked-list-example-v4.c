...
int main(void)
{
	...
	/* Tail */
	c = theListPtr->head->next->next->next->next->next->next->next->c;
	if(delete(&theListPtr, c)) {
		printf("%c deleted.\n", c);
	}
	printLinkedList(theListPtr);
...
struct list* createList()
{
	...
	if (theListPtr != NULL) {
		...
		if (theListPtr->head == NULL && theListPtr->size == 0) {
			return theListPtr;
		}
	}
...
void initialiseList(struct list** theListPtr2Ptr)
{
	/*
	 * Set up a new empty list.
	 */
	(*theListPtr2Ptr)->head = NULL;
	(*theListPtr2Ptr)->size = 0;

	/*
	 * 0 out all the memory of the list.
	 */
	memset(*theListPtr2Ptr, 0, sizeof(struct list));
}

enum BOOLEAN insert(struct list** theListPtr2Ptr, char c, int sortList)
{
	...
	currentNodePtr = (*theListPtr2Ptr)->head;
	...
	/*
	 * The value to insert was greater than or less than the head. So we need
	 * to insert into the head.
	 */
	if (previousNodePtr == NULL && sortList != 0) {
...
enum BOOLEAN delete(struct list** theListPtr2Ptr, char c)
{
	struct node* previousNodePtr;
	struct node* currentNodePtr;
	...
	if(currentNodePtr->c == c) {
		(*theListPtr2Ptr)->head = currentNodePtr->next;
		free(currentNodePtr);
		return TRUE;
	}
...
void destroyList(struct list** theListPtr2Ptr)
{
	struct node* currentNodePtr;
