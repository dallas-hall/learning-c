...
void knightsTourAccessibilityMatrixNoLookAhead(size_t boardSize, int board[][boardSize], const int verticalMovements[],
									const int horizontalMovements[], int startRow, int startColumn,
									int accessibilityMatrix[][boardSize]);

void knightsTourAccessibilityMatrixWithLookAhead(size_t boardSize, int board[][boardSize], const int verticalMovements[],
											   const int horizontalMovements[], int startRow, int startColumn,
											   int accessibilityMatrix[][boardSize]);
...
int main(void)
{
	...
	puts("### Knights Tour Graph - No Lookahead.");
	// 0 out the array.
	memset(chessBoard, -1, sizeof(chessBoard));
	for (int i = 0; i < BOARD_SIZE; i ++) {
		for (int j = 0; j < BOARD_SIZE; j ++) {
			...
			knightsTourAccessibilityMatrixNoLookAhead(BOARD_SIZE, chessBoard, verticalMovements, horizontalMovements, i,
	...
	puts("### Knights Tour Graph - With Lookahead.");
	// 0 out the array.
	memset(chessBoard, -1, sizeof(chessBoard));
	for (int i = 0; i < BOARD_SIZE; i ++) {
		for (int j = 0; j < BOARD_SIZE; j ++) {
			...
			knightsTourAccessibilityMatrixWithLookAhead(BOARD_SIZE, chessBoard, verticalMovements, horizontalMovements, i,
										   j, accessibilityMatrix);
			printf("Chess board final state %d,%d.\n", i, j);
			print2dArray(BOARD_SIZE, BOARD_SIZE, chessBoard);
			puts("Knight your graph accessibility matrix final state.");
									...
									int accessibilityMatrix[][boardSize])
{
	...
	for (size_t moveCounter = 1; moveCounter < MAX_MOVES; moveCounter++) {
		...
		if (moveFound) {
			...
			// Update all squares that are accessible from this square, and reduce by one.
			for (size_t nextMove = 0; nextMove < POSSIBLE_MOVES; nextMove++) {
											   ...
											   int accessibilityMatrix[][boardSize])
{
	...
	int currentBestAccessibilityValue = INT_MAX;
	/*
	 * Store valid moves and their accessibility number.
	 * Used later to iterate through the lowest ties and perform a lookahead
	 */
	int allValidMoves[POSSIBLE_MOVES][2];
	memset(allValidMoves, -1, sizeof(allValidMoves));
	...
	for (size_t moveCounter = 1; moveCounter < MAX_MOVES; moveCounter++) {
		...
		for (size_t moveNumber = 0; moveNumber < POSSIBLE_MOVES; moveNumber++) {
			...
			if (moveStaysOnBoard && moveHasntBeenDoneYet) {
				int currentMoveAccessibilityValue = accessibilityMatrix[tempRow][tempColumn];
				allMoves
				// Get the lowest accessibility move
				if (currentMoveAccessibilityValue < currentBestAccessibilityValue && currentMoveAccessibilityValue > 0) {
					// Reset back to the move numbers start row
					currentBestRow = currentRow;
					currentBestColumn = currentColumn;
				...
				// Handle ties, but using a single lookahead to find out which move is better.
				else if (currentMoveAccessibilityValue == currentBestAccessibilityValue && currentMoveAccessibilityValue > 0) {
