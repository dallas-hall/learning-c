...
{
	int a[SIZE];
	int i, searchValue, position, searches, found, low, middle, high;
	...
	if(found) {
		printf("The search value %d was first found at position %d after %d searches.\n", searchValue, position, searches);
	}
	else {
